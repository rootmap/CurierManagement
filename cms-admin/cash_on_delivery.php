...
if (isset($_POST['create'])) {
    extract($_POST);
    if (!empty($division_id) && !empty($district_id) && !empty($price)) {
        $insert = array('division_id' => $division_id, 'district_id' => $district_id, 'price' => $price, 'date' => date('Y-m-d'), 'status' => 1);
    ...
    if (!empty($division_id) && !empty($district_id) && !empty($price)) {
        ...
        $upd2 = array('division_id' => $division_id, 'district_id' => $district_id, 'price' => $price, 'date' => date('Y-m-d'), 'status' => 1);
...
<!--[if !IE]><!--><html><!-- <![endif]-->
    <head><?php
        echo $plugin->softwareTitle("Cash On Delivery");
    ...
    <body class="">
        ...
        <div id="content">
            <h1 class="content-heading bg-white border-bottom">Cash On Delivery</h1>
            <div class="innerAll bg-white border-bottom">
                <ul class="menubar">
                    <li class="active"><a href="#">Create New Cash On Delivery</a></li>
                    <li><a href="cash_on_delivery_data.php">Cash On Delivery List</a></li>
                </ul>
            </div>
                ...
                <div class="widget widget-inverse" >
                    ...
                    if (isset($_GET['edit'])) {
                        ...
                        <div class="widget-head">
                            <h4 class="heading">Update/Change - Cash On Delivery</h4>
                        </div>
                        <!-- // Widget heading END -->
                                        ...
                                        <select id='form-field-1' name='division_id' class='form-control'><option value='0'>Please Select</option><?php
                                            ...
                                            if (!empty($sqldivision_id)) {
                                                foreach ($sqldivision_id as $division_id):
                                                    ?><option <?php if ($division_id->id == $ex_division_id_data) { ?> selected='selected' <?php } ?> value='<?= $division_id->id ?>'><?= $division_id->name ?></option><?php endforeach; ?><?php } ?></select>
                                    </div>
                                        ...
                                        <select id='form-field-1' name='district_id' class='form-control'><option value='0'>Please Select</option><?php
                                            ...
                                            if (!empty($sqldistrict_id)) {
                                                foreach ($sqldistrict_id as $district_id):
                                                    ?><option <?php if ($district_id->id == $ex_district_id_data) { ?> selected='selected' <?php } ?> value='<?= $district_id->id ?>'><?= $district_id->name ?></option><?php endforeach; ?><?php } ?></select>
                                    </div>
                                </div><div class='form-group'>
                    ...
                    <?php } else { ?>
                        <!-- Widget heading -->
                        <div class="widget-head">
                            <h4 class="heading">Create New Cash On Delivery</h4>
                        </div>
                        <!-- // Widget heading END -->
