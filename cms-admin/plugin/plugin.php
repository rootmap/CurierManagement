...
//    public function TextAreaCssOld() {
        ...
        $content = '<script>
                    $(document).ready(function () {
                        $("textarea").kendoEditor({
                            imageBrowser: {
                                transport: {
                                    read: "'.$this->baseUrl('upload/Read/index.php').'",
                                    destroy: {
                                        url: "'.$this->baseUrl('upload/Read/destroy.php').'",
                                        type: "POST"
                                    },
                                    create: {
                                        url: "'.$this->baseUrl('upload/Read/create.php').'",
                                        type: "POST"
                                    },
                                    thumbnailUrl: "'.$this->baseUrl('upload/Read/index.php').'",
                                    uploadUrl: "'.$this->baseUrl('upload/Read/upload.php').'",
                                    imageUrl: "'.$this->baseUrl('upload/Read').'/{0}"
                                }
                            },
                            tools: [
                                "bold",
...
';
    ...
    function baseUrl($suffix = '') {
        ...
        $web_root = $protocol . $_SERVER['HTTP_HOST'] . "/" . "fakhrul/quriar/quriar/cms-admin/";
        $suffix = ltrim($suffix, '/');
    ...
    public function softwareTitle($title = '') {
        if ($title != '') {
            $content = '<title>' . $title . ' |  Formula-CMS (CRUD Manager) | By Root Map</title>';
            return $content;
        } else {
            $content = '<title>Formula-CMS (CRUD Manager) | By Root Map</title>';
            return $content;
        }
    }
