...
class db_class {
    ...
    public function open() {
        $con=mysqli_connect("localhost", "root", "", "spxce_quriar");
        return $con;
    }

    public function pdocon() {
        $db=new PDO("mysql:host=localhost;dbname=spxce_quriar;", "root", "");
    ...
    function LbaseUrl($suffix='') {
        $protocol=strpos($_SERVER['SERVER_SIGNATURE'], '443') !== false ? 'https://' : 'http://';
        $web_root=$protocol . $_SERVER['HTTP_HOST'] . "/" . "fakhrul/quriar/quriar/";
        $suffix=ltrim($suffix, '/');
        return $web_root . trim($suffix);
    }
